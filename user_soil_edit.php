...
if(!empty($user))
{
  $quantity = "SELECT * from soil where soil_code = '$soil_code'";
  $quantity1 = mysqli_query($conn,$quantity);
  if($left = mysqli_fetch_array($quantity1)){
    $quantity2 = $left['quantity'] - 1;
  }
  $quantity3 = "UPDATE soil SET quantity='$quantity2' where soil_code = '$soil_code'";
...
$query = "INSERT into online_delivery(order_status,user_email,employee_email,soil_code) 
values('processing','$user','harrypoter@gmail.com','$soil_code')";

$result = mysqli_query($conn,$query);
if($result){
header("Location:user_soil_edit1.php");
die;
}
}
else
